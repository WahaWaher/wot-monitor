...
const filterMsgs = (messages = [], filter = []) => {
...
const AppNotifications = () => {
  ...
  return (
    <Root>
          ...
          <div className="d-flex alighn-items-center">
            <div className="mr-3">
              <ButtonIconed
                size="sm"
                type="light"
                active={isInMsgTypes('success', noticesTypeFilter)}
                tooltip="Вкл./Откл. отображение успешных действий"
                renderIcon={() => <Icon path={mdiCheckCircleOutline} />}
                onClick={() => toggleNoticesTypeFilter('success')}
              />
              {/* <ButtonIconed
                size="sm"
                type="light"
                active={isInMsgTypes('info', noticesTypeFilter)}
                tooltip="Вкл./Откл. отображение информационных сообщений"
                renderIcon={() => <Icon path={mdiInformationOutline} />}
                onClick={() => toggleNoticesTypeFilter('info')}
              /> */}
              <ButtonIconed
                size="sm"
                type="light"
                active={isInMsgTypes('error', noticesTypeFilter)}
                onClick={() => toggleNoticesTypeFilter('error')}
              ...
              {/* <ButtonIconed
                size="sm"
                type="light"
                active={isInMsgTypes('warning', noticesTypeFilter)}
                onClick={() => toggleNoticesTypeFilter('warning')}
              ...
              <ButtonIconed
                size="sm"
                type="light"
                active={isInMsgTypes('reserve', noticesTypeFilter)}
                onClick={() => toggleNoticesTypeFilter('reserve')}
            ...
            {dialogDelAllMsgs.renderDialog(dialogDelAllMsgs)}
          </div>
        )}
      />
      <Container>
        {visibleMsgs.items.map((item) => {
          ...
          return (
            ...
            >
              {message}
              {/* <small className="d-block mt-1" style={{ opacity: 0.5 }}>
              {name}
            </small> */}
            </Message>
          );
      ...
      {dialogDelMsg.renderDialog(dialogDelMsg)}
      {dialogMsgDetail.renderDialog(dialogMsgDetail)}
    </Root>
  );
};
