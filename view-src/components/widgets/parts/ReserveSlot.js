...
const ReserveSlot = ({
  ...
  <Root {...rest}>
    <Container size={size}>
      <BgProgress />
      <ReserveIcon active={active} data-tip data-for={`reserve-${type}`}>
        {reserves[type]?.icon && (
          <img src={reserves[type].icon} alt="reserve" />
        )}
      </ReserveIcon>
      <Tooltip id={`reserve-${type}`} delayShow={500} delayHide={0} maxWidth={225}>
        {rednderTooltip()}
      </Tooltip>
...
ReserveSlot.propTypes = {
  ...
  timeLeft: PropTypes.oneOfType([PropTypes.string, PropTypes.bool]),
  rednderTooltip: PropTypes.func,
